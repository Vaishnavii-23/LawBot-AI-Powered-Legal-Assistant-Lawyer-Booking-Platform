...
import { deleteAdminAdvocate, fetchAdminAdvocates } from "../lib/apiClient.js";
...
const AdminAdvocates = () => {
  ...
  const loadAdvocates = async () => {
    ...
    try {
      ...
      setError(err.message || "Unable to load advocates");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setAdvocates((prev) => prev.filter((item) => item.id !== id));
    } catch (err) {
      alert(err.message || "Could not delete advocate.");
    }
  };

  const filtered = useMemo(() => {
    const value = query.trim().toLowerCase();
    if (!value) return advocates;
        ...
        <div className="rounded-3xl bg-white p-6 shadow-soft">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <h1 className="mt-2 text-2xl font-semibold text-slate-900">Manage Advocates</h1>
              <p className="mt-2 text-sm text-slate-500">Monitor advocate accounts and remove inactive profiles.</p>
            </div>
            <button
              type="button"
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            />
            <span className="text-xs text-slate-400">{filtered.length} advocates</span>
          </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="flex h-9 w-9 items-center justify-center rounded-full bg-purple-100 text-sm font-semibold text-purple-700">
                      {(advocate.full_name || advocate.email || "A").charAt(0).toUpperCase()}
                    </div>
                    <div>
                      <p className="font-semibold text-slate-900">{advocate.full_name || "—"}</p>
                  ...
                  <button
                    ...
                    aria-label={`Delete ${advocate.email}`}
                  >
                    <Trash2 size={16} />
                  </button>
                </div>
